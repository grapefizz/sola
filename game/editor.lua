...
function Editor:scrollDropdown(delta)
    self.loadDropdownScroll = math.max(0, math.min(self:maxDropdownScroll(), self.loadDropdownScroll + delta))
end
...
function Editor:isOverHud(x, y)
    ...
    return x >= HUD_X and x <= HUD_X + HUD_W and y >= HUD_Y and y <= HUD_Y + HUD_H
end
...
function Editor:draw(grid, camera)
    ...
    love.graphics.rectangle("line", spawnX - spawnSize / 2 + 3, spawnY - spawnSize / 2 + 3, spawnSize - 6, spawnSize - 6)
    ...
    if self.loadDropdownOpen then
        ...
        for i = 1, visible do
            ...
            local hovered = mouseX >= dx and mouseX <= dx + listW and mouseY >= itemY and mouseY < itemY + DROPDOWN_ITEM_H
    ...
    if self.status ~= "" and not self.namingOpen then
        ...
        love.graphics.rectangle("fill", 10, 202, 280, 24, 4, 4)
        love.graphics.setColor(0.85, 0.93, 1)
        love.graphics.print(self.status, 18, 202 + textY)
    end
